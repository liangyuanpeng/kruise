...
func DiscoverGVK(gvk schema.GroupVersionKind) bool {
	...
	if err != nil {
		if err == errKindNotFound {
			klog.InfoS("Not found kind in group version.", "kind", gvk.Kind, "groupVersion", gvk.GroupVersion().String(), "waitingTime", time.Since(startTime))
			return false
		}

		// This might be caused by abnormal apiserver or etcd, ignore it
		klog.ErrorS(err, "Failed to find resources in group version", "groupVersion", gvk.GroupVersion().String(), "waitingTime", time.Since(startTime))
	}
...
func DiscoverObject(obj runtime.Object) bool {
	...
	if err != nil {
		//TODO lan
		klog.ErrorS(err, "Not recognized object in scheme", "object", "klog.KObj(obj)")
		return false
	}
